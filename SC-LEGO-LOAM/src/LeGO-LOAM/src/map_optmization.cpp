...
namespace lego_loam
{
    ...
    void mapOptimization::transformAssociateToMap()
    {
        ...
        float srzcrx = (cbcz * sbcy - cbcy * sbcx * sbcz) * (calx * salz * (cblz * sbly - cbly * sblx * sblz) - calx * calz * (sbly * sblz + cbly * cblz * sblx) +
                                                             cblx * cbly * salx) -
                       (cbcy * cbcz + sbcx * sbcy * sbcz) * (calx * calz * (cbly * sblz - cblz * sblx * sbly) - calx * salz * (cbly * cblz + sblx * sbly * sblz) +
                                                             cblx * salx * sbly) +
                       cbcx * sbcz * (salx * sblx + calx * cblx * salz * sblz + calx * calz * cblx * cblz);
        float crzcrx = (cbcy * sbcz - cbcz * sbcx * sbcy) * (calx * calz * (cbly * sblz - cblz * sblx * sbly) - calx * salz * (cbly * cblz + sblx * sbly * sblz) +
                                                             cblx * salx * sbly) -
                       (sbcy * sbcz + cbcy * cbcz * sbcx) * (calx * salz * (cblz * sbly - cbly * sblx * sblz) - calx * calz * (sbly * sblz + cbly * cblz * sblx) +
                                                             cblx * cbly * salx) +
    ...
    bool mapOptimization::detectLoopClosure()
    {
        ...
        // find the closest history key frame  这里用的是这种关键帧位姿查询？ 不是用SC吗
        std::vector<int> pointSearchIndLoop;                   //  搜索完的邻域点对应的索引
        std::vector<float> pointSearchSqDisLoop;               //  搜索完的邻域点与当前点的欧氏距离
        kdtreeHistoryKeyPoses->setInputCloud(cloudKeyPoses3D); //每隔0.3米的位置选择一个关键帧 存入到cloudKeyPoses3D
        ...
        for (int i = 0; i < pointSearchIndLoop.size(); ++i)
        {
            ...
            if (abs(cloudKeyPoses6D->points[id].time - timeLaserOdometry) > 30.0)
            {
                ...
                // break;  找一个最老的一帧 与这个帧进行匹配 我觉得这个应该找最近的 而不是几个点的最老点
                if (id < curMinID)
                {
                    curMinID = id;
        ...
        {
            //  检测到回环了会保存四种点云  这里的检测回环是根据时间超过30秒 才回环 即 同样地方发现帧的时间超过30s了 开始回环
            // save latest key frames
            latestFrameIDLoopCloure = cloudKeyPoses3D->points.size() - 1;
            // 点云的xyz坐标进行坐标系变换(分别绕xyz轴旋转) 所有点云和位姿全部分开了
            ...
            *RSlatestSurfKeyFrameCloud += *transformPointCloud(surfCloudKeyFrames[latestFrameIDLoopCloure],
            ...
            // 滤掉latestSurfKeyFrameCloud中index<0的点??? index值会小于0? 应该不会的
            ...
            *RSlatestSurfKeyFrameCloud = *RShahaCloud;
            ...
            // historyKeyframeSearchNum 在utility.h中定义为25，前后25个点进行变换  把关键帧前后25帧全部组成一个map
            ...
            downSizeFilterHistoryKeyFrames.filter(*RSnearHistorySurfKeyFrameCloudDS); //SCnearHistorySurfKeyFrameCloudDS
        }
        ...
        *SClatestSurfKeyFrameCloud += *transformPointCloud(surfCloudKeyFrames[latestFrameIDLoopCloure],
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            if ((int)SClatestSurfKeyFrameCloud->points[i].intensity >= 0) //同样这里的instensity应该不会0
            {
    ...
    bool mapOptimization::LMOptimization(int iterCount)
    {
        ...
        for (int i = 0; i < laserCloudSelNum; i++)
        {
            ...
            float arz =
                ((crz * srx * sry - cry * srz) * pointOri.x + (-cry * crz - srx * sry * srz) * pointOri.y) * coeff.x + (crx * crz * pointOri.x - crx * srz * pointOri.y) * coeff.y +
    ...
    void mapOptimization::saveKeyFramesAndFactor()
    {
        ...
        if (sqrt((previousRobotPosPoint.x - currentRobotPosPoint.x) * (previousRobotPosPoint.x - currentRobotPosPoint.x) +
                 (previousRobotPosPoint.y - currentRobotPosPoint.y) * (previousRobotPosPoint.y - currentRobotPosPoint.y) + (previousRobotPosPoint.z - currentRobotPosPoint.z) * (previousRobotPosPoint.z - currentRobotPosPoint.z)) < 0.3)
        ...
        { // v2 uses downsampled raw point cloud, more fruitful height information than using feature points (v1)
            //  这里对点云提取scan context特征  这边是用全部点云
