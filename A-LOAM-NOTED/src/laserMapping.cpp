...
void BFS(PointType point, pcl::KdTreeFLANN<PointType> KdTree, int &countBFS, pcl::PointCloud<pcl::PointXYZI> &cloud, std::vector<int> &indice_out)
{
	std::vector<int> indices;
	std::vector<float> Distance;
	KdTree.nearestKSearch(point, 5, indices, Distance);
	countBFS++;
	indice_out.push_back(indices[0]);
	// std::cout<<point.x<<point.y<<std::endl;
	for (unsigned int i = 1; i < indices.size(); i++)
	{
		if (Distance[i] < 2 && cloud.points[indices[i]].intensity != 1)
		{
			cloud.points[indices[i]].intensity = 1;
			BFS(cloud[indices[i]], KdTree, countBFS, cloud, indice_out);
		}
	}
}
void process()
	...
	while (1)
	{
			   ...
			   !fullResBuf.empty() && !odometryBuf.empty())
		{
			...
			if (laserCloudCornerFromMapNum > 10 && laserCloudSurfFromMapNum > 50)
			{
				TicToc t_opt;
				TicToc t_tree;
				//将submap的角点和平面点放在kdtree里面 这样就可以找临近点 放入kdtree
				...
				for (int iterCount = 0; iterCount < 2; iterCount++)
				{
					...
					for (int i = 0; i < laserCloudCornerStackNum; i++)
					{
						...
						pointAssociateToMap(&pointOri, &pointSel);//终于找到你了
			...
			// Mapping做准备
			//q w_curr是一个EIGEN::MAP的，然后要用一个指针来构造Map
			//这样 baselink->odom是 odom计算出来的，然后经过优化后可以的到 odom->map 这是我们这里精优化得到的
			//最后就是baselink->map
			transformUpdate();
			...
			//每5帧发送一个map  对于我们那个door_last.world的地图来说 一个map就足够了

			if (frameCount % 5 == 0)
			{
				laserCloudSurround->clear();
				pcl::PointCloud<PointType> out_WallGround;
				pcl::PointCloud<PointType> out_Else;
				out_WallGround.clear();
				for (int i = 0; i < laserCloudSurroundNum; i++)
				{
					//laserCloudSurroundInd这个里面存的就是当前cube附近的的几个cube的下标
					int ind = laserCloudSurroundInd[i];
					//laserCloudCornerArray 存放着附近的cube所有的点云
					out_Else += *laserCloudCornerArray[ind];
					*laserCloudSurround += *laserCloudCornerArray[ind];
					out_WallGround += *laserCloudSurfArray[ind];
			...
			//每20帧输出一次全局地图(其实并不是全局的 只是4851的cube 每一个cube 50x50)
			if (frameCount % 20 == 0)
			{
