    ...
    while (ros::ok())
    {
            ...
            !fullPointsBuf.empty())
        {
            ...
            else
            {
                ...
                TicToc t_opt;
                //! LegoLoam里面由25次 是有权重影响因子的  每次更新para_q para_t
                for (size_t opti_counter = 0; opti_counter < 2; ++opti_counter)// 点到线以及点到面的ICP，迭代2次
                {
                    ...
                    {
                        // 将当前帧的corner_sharp特征点O_cur，从当前帧的Lidar坐标系下变换到上一帧的Lidar坐标系下（记为点O，注意与前面的点O_cur不同），以利于寻找corner特征点的correspondence
                        ...
                        if (pointSearchSqDis[0] < DISTANCE_SQ_THRESHOLD)// 如果最近邻的corner特征点之间距离平方小于阈值，则最近邻点A有效
                        {
                            ...
                            // 按照scan的顺序提取的，所以laserCloudCornerLast中的点也是按照scanID递增的顺序存放的 对的！ 
                            ...
                            //上面两个for其实是在前后两个点找出一个最小的 用minPointSqDis2参数来判断哪个更小！！！！！！！！！！！！！！
                        }
                        if (minPointInd2 >= 0) // both closestPointInd and minPointInd2 is valid
                        {                      // 即特征点O的两个最近邻点A和B都有效
                            //这个curr是当前帧的点  所以我觉得这里应该错了  应该是pointsel 因为这个
                            Eigen::Vector3d curr_point(cornerPointsSharp->points[i].x,
                                                       cornerPointsSharp->points[i].y,
                                                       cornerPointsSharp->points[i].z);
                            //下面a是 kdtree找出来的临近点  b是两个for找出来的最近点
                            //a,b都是当前点转化到上一帧坐标系下寻找到的最近两个点
                ...
                t_w_curr = t_w_curr + q_w_curr * t_last_curr;//* 里程计叠加
            ...
            laserCloudCornerLast = laserCloudTemp;//每一帧 在这里赋值  不知道为什么不在那个标志位systemInited 赋值 吃饱了撑得
